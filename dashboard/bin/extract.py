...
def get_gwent_data(url):
    ...
    output = {
        "player": "".join(
            re.findall(
                r'<strong class="l-player-details__name">\\n\s+(.*?)</strong>',
                str(html),
            )
        ),
        "mmr": "".join(
            re.findall(
                r'<div class="l-player-details__table-mmr">.*?<strong>(.*?)</strong></div>',
                str(html),
            )
        ).replace(",", ""),
        "position": "".join(
            re.findall(
                r'<div class="l-player-details__table-position">.*?<strong>(.*?)</strong></div>',
                str(html),
            )
        ).replace(",", ""),
        "rank": "".join(
            re.findall(
                r'<span class="l-player-details__rank"><strong>(.*?)</strong></span>',
                str(html),
            )
        ),
        "ladder": "".join(
            re.findall(
                r'<div class="l-player-details__table-ladder" ><span>(.*?)</span></div>',
                str(html),
            )
        ),
    }
        ...
        with urllib.request.urlopen(url, context=ssl_context) as response:
            ...
            if "nextepisode" in links.keys():
                with urllib.request.urlopen(
                    links["nextepisode"]["href"], context=ssl_context
                ) as episode_response:
